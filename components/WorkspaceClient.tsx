...
import { CREDIT_COST_PER_GENERATION, MIN_CREDITS_TO_GENERATE } from "@/lib/constants";
...
export function WorkspaceClient({
  ...
  const [credit, setCredit] = useState(userCredits);
  ...
  const handleStop = useCallback(()=>{
    ...
    const handleImprove = useCallback(async(userRequest: string) =>{
    if(isGenerating || isImproving) return;
    if(credits < CREDIT_COST_PER_GENERATION) return;
    if(!workspaceIdRef.current) return;

    // read filedata from ref
