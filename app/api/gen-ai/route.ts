...
export async function POST(request: NextRequest){
    ...
    async function validateDependencies(dep: Record<string, string>): Promise<Record<string, string>> {
        const valid: Record<string, string> = {};
        await Promise.all(
            Object.entries(dep).map(async ([pkg, version]) =>{
                try {
    ...
    if(user.credits < CREDIT_COST_PER_GENERATION){
        return Response.json({message: " Not enough credits"}, {status: 402})
    }

    const encoder = new TextEncoder();
    const stream = new ReadableStream({
        async start(controller) {
            const enqueue = (chunck: string) =>
            ...
            try{
                ...
                let accumulate = ""; // collect the chunks of data
                let lastEmitTime = 0; // track the last time we emitted data
            ...
            enqueue(sseEvent("status",{message: "Saving to database..."}));
            ...
            select: { credits: true },
                ...
                enqueue(
                    sseEvent("error", {
                        message: "An error occurred during AI generation. Please try again.",
                    })
                );
            }
            finally{
